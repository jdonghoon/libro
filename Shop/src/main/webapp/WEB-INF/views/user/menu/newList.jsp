  ...
  <script>	
	  $(document).ready(function() {
		  $('a[name="categoryType"]').on('click', function(e) {
		        ...
		        // ajax 요청
				$.ajax({
		            url: "category.do", // 서버 API 엔드포인트
		            type: "GET",
		            data: { categoryType: categoryType }, // 카테고리를 데이터로 전송
		            dataType: "json",
		            headers: {
		                "Accept": "application/json" // JSON 요청임을 명시
		            },
		            success: function(response) {
		                console.log(response);

		                // 기존 컨텐츠 초기화
		                $('#categotry-content').empty();

		                if (response.length === 0) {
		                    $('#categotry-content').append('<div class="emptyData">선택하신 카테고리에 해당하는 상품이 없습니다.</div>');
		                }
		                ...
		                $('#categotry-content').append(productHtml);
		                ...
		                let pagingHtml = `
		                  ...
		                  $('#categotry-content').append(pagingHtml);
